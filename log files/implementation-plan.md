...
**Done: Phases 0, 1, 2, 3, 4, plus a full UI design pass. Next up: Phase 5 (Insights engine).**
...
**Immediate next step:** Phase 5 - build `lib/insights/{engine,topCategories,momTrend,budgetOverage,suggestions}.ts` (pure, testable heuristics, no DB access inside them, no LLM calls) and the `/insights` page. Construct known scenarios by hand (e.g. a category exactly 30% up month-over-month) and confirm the generated insight text/numbers match hand-calculated expectations exactly before trusting the feature - the plan already flags this as the highest-stakes phase for correctness, since it's the app's one "smart" feature and needs to stay fully auditable back to real numbers.
...
5. ⬜ **Insights engine** - heuristic modules + `/insights` page, sanity-checked against real data from prior phases. **← next up.**
