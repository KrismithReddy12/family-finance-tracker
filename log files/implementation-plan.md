...
**Done: Phases 0, 1, 2, 3, 4, 5, plus a full UI design pass. Phase 6 (PWA polish) is code-complete but blocked partway through - see below.**
...
**Phase 6 status - blocked partway through, waiting on the user:** every piece of PWA polish that's pure code has been written and verified as far as this environment allows (manifest, icons, service worker, offline banner, offline-mutation guard - all confirmed serving correctly under a real production build, not just `next dev`). What's left needs things this environment doesn't have: a Vercel account to deploy to (HTTPS is a hard requirement for service workers and "Add to Home Screen" - there's no way to test real installability on `localhost`), a real Neon database for that production deploy (local dev has been Postgres via `prisma dev` this whole time, never Neon), and **a physical iPhone** to verify Add to Home Screen, standalone launch (no browser chrome), and Airplane Mode behavior - desktop emulation cannot substitute for iOS Safari's real PWA quirks (see Verification Approach below). None of this was attempted without asking first, since it means creating/using external accounts and making the app briefly (or permanently) publicly reachable - both decisions belong to the user, not something to do unilaterally.

**Immediate next step:** once the user decides how they want to handle the Vercel/Neon/iPhone pieces, finish Phase 6's verification, then move to Phase 7 (settings & hardening): password change, profile management (block deleting a profile with expense history), empty states, error boundaries.
...
6. 🟡 **PWA polish** - code-complete (manifest, icons, service worker, offline banner, offline-mutation guard), verified as far as `next build`/`next start` allow. **Blocked on the user** for Vercel deploy + real Neon database + physical iPhone install testing - see Progress log above. **← here.**
